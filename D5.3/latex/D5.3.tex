...
\end{IEEEeqnarray}
where $^{\mathcal{I}} v_c \in \mathbb{R}^3$ denotes the velocity of the point $p_M$, and $^{\mathcal{I}} \omega_c \in \mathbb{R}^3$ the angular velocity of the flat plate, both expressed with respect to the inertial frame. The above equations point out that the flat plate can only rotate only about the axes $\imath$ and $\jmath$. Note that the constraints~\eqref{constraintsWithVerticalMotion} can be compactly written as:
\begin{IEEEeqnarray}{RCL}
...
Assume that the robot is balancing, on two feet, on the compliant carpet described in Section~\ref{sec:modelCarpet}. In view of Eq.~\eqref{constraintsWithVerticalMotionCompactDetails}, the constraints acting on the robot can be compactly written as:
\begin{IEEEeqnarray}{RCL}
\label{constraintsWithVerticalMotionCompact}
 \IEEEyesnumber
 \bar{H}(\imath,\jmath,n) ^{\mathcal{I}} {v}_f &=& 0, 
\end{IEEEeqnarray}
with 
\begin{IEEEeqnarray}{RCL}
	\IEEEyesnumber
	\bar{H} &=& 
	\begin{pmatrix}
		\bar{H}_L(\imath_L,\jmath_L,n_L) && 0_{4\times 6} \\
 		0_{4\times 6} && \bar{H}_R(\imath_R,\jmath_R,n_R)
	\end{pmatrix} \IEEEyessubnumber  \\
	\label{feetvelocity}
 v_f &=& 
 \begin{pmatrix}
...
\end{pmatrix} \IEEEyessubnumber
\end{IEEEeqnarray}
and  $\bm{v}_L$ and $\bm{v}_R$ the linear and angular velocity of the central point of the robot's feet, respectively. Now, the feet velocity $ ^{\mathcal{I}}{v}_f$ can be expressed in terms of the robot velocity $\nu$ though its Jacobian, i.e. 
\[ ^{\mathcal{I}}{v}_f = J(q)\nu.\]
...
\label{constraintsCompactJBar}
 \IEEEyesnumber
 \bar{J}(q) &:=& \bar{H}(\imath,\jmath,n)J(q). 
\end{IEEEeqnarray}
Hence, the constraint equations when the robot stands on a compliant carpet have the same form of  those when standing on rigid constraints (compare Eqs.~\eqref{constraintsCompactComplJBar} and~\eqref{constraintsRigid}). What does change between these two situations is the number of rigid constraints acting on the system. In fact, in the case the robot stands on rigid contacts, one has 12 kinematic constraints, while when the humanoid balances on the soft carpet described in  Section~\ref{sec:modelCarpet}, one has eight rigid constraints acting on the system. These eight rigid constraints generate eight contact \emph{forces} $f_{rigid} \in \mathbb{R}^8$. The contact forces depend on the torque input $\tau$ via the dynamic equation
\begin{IEEEeqnarray}{RCL}
    \label{eq:system_dynamicsCarpet}
       {M}(q)\dot{{\nu}} + {C}(q, {\nu}) {\nu} + {G}(q) &=&  B \tau + J^\top(q) f_{rigid}
       + J^\top(q) f_{ext} \IEEEyessubnumber
\end{IEEEeqnarray}
and the constraint equation~\eqref{constraintsCompactComplJBar}.
......
   Recall also that the control objective is the asymptotic stabilization of the robot's momentum $H$, the rate-of-change of which equals the summation of all external wrenches acting on the system. In view of~\eqref{centroidalMomentumDyn}, one has
\begin{IEEEeqnarray}{RCL}
	...
	f,
\end{IEEEeqnarray}
where now $f$ is composed of two components: the wrench due to the compliant carpet, and the wrench necessary for the satisfaction of the constraints 

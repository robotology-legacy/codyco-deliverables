...
Assume that the robot is balancing on both feet, underneath which there is the compliant carpet characterized by the equations presented in Section~\ref{sec:modelCarpet}. In view of Eq.~\eqref{constraintsWithVerticalMotionCompactDetails}, the constraints acting on both feet  can be compactly written as:
\begin{IEEEeqnarray}{RCL}
\label{constraintsWithVerticalMotionCompact}
 \IEEEyesnumber
...
one has 8 rigid constraints acting on the system. These rigid constraints generate eight contact \emph{forces} $f_{rigid} \in \mathbb{R}^8$, which must be taken into account in the dynamic equation, i.e.
\begin{IEEEeqnarray}{RCL}
    \label{eq:system_dynamicsCarpet}
       {M}(q)\dot{{\nu}} + {C}(q, {\nu}) {\nu} + {G}(q) &=&  B \tau 
       + \bar{J}^\top(q) f_{rigid} + J^\top(q) f_{comp} \IEEEyessubnumber
\end{IEEEeqnarray}
By combining the above equation with the constraints~\eqref{constraintsCompactComplJBar}, one can find an expression of the contact forces $f_{rigid}$, which thus depend on the  torque $\tau$, and can be assumed as an artificial control input. Note that the effect of the compliant forces $f_{comp}$ in Eq.~\eqref{eq:system_dynamicsCarpet} is also taken into account. More precisely, in view of Eq.~\eqref{forceTorqueOn3DBentPlate}, one has

\begin{IEEEeqnarray}{RCL}
\label{compliantForces}
\label{force3D}
F &=& kld|n^\top e_3|\left(h-z_M \right)e_3 \\
\label{torque3D}
M &=& S(p_M - \bar{p})F + \frac{kld}{12}|n^\top e_3|S(e_3)\Lambda(\imath,\jmath)e_3 
\end{IEEEeqnarray}
 for the control of the robot momentum, i.e. in the equation

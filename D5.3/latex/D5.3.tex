...
Assume that the robot is balancing, on two feet, on the compliant carpet described in Section~\ref{sec:modelCarpet}. In view of Eq.~\eqref{constraintsWithVerticalMotionCompactDetails}, the constraints acting on the system can be compactly written as:
\begin{IEEEeqnarray}{RCL}
\label{constraintsWithVerticalMotionCompact}
 \IEEEyesnumber
...
\begin{IEEEeqnarray}{RCL}
	...
	\bar{H} &=& 
	\begin{pmatrix}
		\bar{H}_L(\imath_L,\jmath_L,n_L) && 0_{3\times3} \\
 		0_{3\times3} && \bar{H}_R(\imath_R,\jmath_R,n_R)
	\end{pmatrix} \IEEEyessubnumber  \\
	\label{feetvelocity}
 v_f &=& 
 \begin{pmatrix}
...
\end{pmatrix} \IEEEyessubnumber
\end{IEEEeqnarray}
and and $\bm{v}_L$ and $\bm{v}_R$ the linear and angular velocity of the central point of the robot's feet, respectively.
